...
class AES:
    ...
    The data is 16 byte/ 128 bit, and is in a list of length 16.
    """
    ...
    def EncryRound(self, data, key):
        ...
        for i in range(0,16,4):
            temp_mixCol1.extend(self.ForwardMixColumn(helpfs.Row2Col(temp_mixCol[i:i+4])))
        #Key addition
    ...
    def CalcForwardSubstitutionByte(self,x,y,return_type='d'):
        ...
        affine_mapping_matrix = genericmatrix.GenericMatrix(size=(8,8),zeroElement=0,identityElement=1,add=self.XOR,mul=self.AND,sub=self.XOR,div=self.DIV)
        affine_mapping_vector = genericmatrix.GenericMatrix(size=(1,8),zeroElement=0,identityElement=1,add=self.XOR,mul=self.AND,sub=self.XOR,div=self.DIV)
        affine_mapping_input  = genericmatrix.GenericMatrix(size=(1,8),zeroElement=0,identityElement=1,add=self.XOR,mul=self.AND,sub=self.XOR,div=self.DIV)
    ...
    def ForwardSBoxGenerator(self):
        ...
        for x in range(16):
            ...
            for y in range(16):
                ...
                hex_y=hex(y)[2:]
                col_index=y+1
                s_box[row_index][col_index]=self.CalcForwardSubstitutionByte(x,y,return_type='h')
    ...
    def InverseSBoxGenerator(self):
        ...
        for x in range(16):
            ...
            for y in range(16):
                ...
                hex_y=hex(y)[2:]
                col_index=y+1
                s_box[row_index][col_index]=self.CalcInverseSubstitutionByte(x,y,return_type='h')
                if x==1 or y==1:  #Intialise the table with its headers
                    s_box[0][col_index] = hex_y
                    s_box[row_index][0] = hex_x
    ...
    def ForwardMixColumn(self, column):
        ...
        if len(column) != 4:
            ...
            if len(column[0]) == 1:
                c= [0,0,0,0]
                for i in range(4):
                    c[i]=((self.GF_256.Multiply(f_mix_col_mat[i][0],column[0][0]))^ \
                         (self.GF_256.Multiply(f_mix_col_mat[i][1],column[1][0]))^ \
                         (self.GF_256.Multiply(f_mix_col_mat[i][2],column[2][0]))^ \
                         (self.GF_256.Multiply(f_mix_col_mat[i][3],column[3][0])))%256
                return c
            else:
    ...
    def InverseMixColumn(self,column):
        ...
        if len(column) != 4:
            ...
            if len(column[0]) == 1:
                c= [0,0,0,0]
                for i in range(4):
                    c[i]=((self.GF_256.Multiply(i_mix_col_mat[i][0],column[0][0]))^ \
                         (self.GF_256.Multiply(i_mix_col_mat[i][1],column[1][0]))^ \
                         (self.GF_256.Multiply(i_mix_col_mat[i][2],column[2][0]))^ \
                         (self.GF_256.Multiply(i_mix_col_mat[i][3],column[3][0])))%256
                return c
            else:
                raise ValueError("Invalid column dim")
